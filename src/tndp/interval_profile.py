"""Daily service interval profile used by the route operating model."""
...
def validate_profile(profile=DEFAULT_INTERVAL_PROFILE) -> None:
    if not profile:
        raise ValueError("Interval profile cannot be empty")
    expected = 14.5
    actual = daily_frequency_factor(profile)
    if abs(actual - expected) > 1e-9:
        raise ValueError(f"Interval profile must total 14.5 peak-frequency hours, got {actual}")
    previous_end = None
    for p in profile:
        if p.hours <= 0 or not 0 < p.frequency_factor <= 1:
            raise ValueError(f"Invalid interval period: {p.name}")
        if previous_end is not None and p.start != previous_end:
            raise ValueError("Interval profile contains a time gap or overlap")
        previous_end = p.end
