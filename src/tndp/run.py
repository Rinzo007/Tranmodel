...
def run_tndp(config=None, *, full_assignment=True, progress=None):
    config = config or NetworkDesignConfig(); config.validate()
    ...
    if full_assignment:
        ...
        def evaluator(route_set):
            ...
            return evaluate_route_set_aequilibrae(route_set, demand, stop_lonlat_xy, project_path, config,
                road_graph=road_graph, stop_mapping=stop_mapping, path_index=path_index, stop_to_zone=stop_to_zone, cache_dir=EVAL_CACHE)
        fast_evaluator = lambda rs: surrogate_evaluator(demand, zone_xy, rs, config, zone_to_stop, stop_xy)
    else:
        evaluator = lambda rs: surrogate_evaluator(demand, zone_xy, rs, config, zone_to_stop, stop_xy); fast_evaluator = evaluator
    ...
    report = {
        ...
        "score": float(ev.score), "base_score": float(meta.get("objective_base_score", ev.score)), "feasible": bool(meta.get("feasible", True)),
        "constraint_violations": meta.get("constraint_violations", []), "objective_components": meta.get("objective_components", {}),
        "user_cost": float(ev.user_cost), "direct_demand_share": float(ev.direct_demand_share), "uncovered_demand": float(ev.uncovered_demand),
        "transfers": float(ev.transfers), "operator_annual_mileage_km": float(ev.operator_cost), "capacity_excess": float(ev.capacity_excess),
        "annual_in_service_hours": float(meta.get("annual_in_service_hours", 0.0)), "fleet": int(meta.get("fleet", 0)),
        "annual_contract_cost_mln": float(meta.get("annual_contract_cost_mln", 0.0)), "annual_amortization_mln": float(meta.get("annual_amortization_mln", 0.0)),
        "route_characteristics": meta.get("route_characteristics", []), "route_set": str(route_path), "route_geojson": str(geojson_path),
        "evaluator": meta.get("evaluator", "unknown"), "full_assignment": bool(full_assignment),
    }
