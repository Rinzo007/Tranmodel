...
def _with_coverage(evaluation: Evaluation, route_set: RouteSet, zone_xy: np.ndarray,
                   zones: gpd.GeoDataFrame, stop_xy: np.ndarray) -> Evaluation:
    """Attach population-weighted coverage to an evaluator result before objective scoring."""
    coverage = population_coverage(
        route_set,
        np.asarray(zone_xy, dtype=float) * 1000.0,
        zones["population"].to_numpy(float),
        np.asarray(stop_xy, dtype=float) * 1000.0,
        radii_m=(400.0, 500.0, 800.0),
    )
    meta = dict(evaluation.metadata or {})
    meta.update(coverage)
    meta["coverage_share"] = float(coverage.get("coverage_800m", 0.0))
    return Evaluation(
        score=evaluation.score,
        user_cost=evaluation.user_cost,
        operator_cost=evaluation.operator_cost,
        uncovered_demand=evaluation.uncovered_demand,
        transfers=evaluation.transfers,
        direct_demand_share=evaluation.direct_demand_share,
        capacity_excess=evaluation.capacity_excess,
        metadata=meta,
    )
...
def run_tndp(config=None, *, full_assignment=True, progress=None):
    ...
    (demand, zones, stops, road_graph, stop_graph, stop_mapping, stop_xy, zone_xy,
     zone_to_stop, zone_to_stops, stop_to_zone, stop_demand, stop_lonlat_xy,
     terminal_nodes) = _load_inputs()
    ...
    candidates = generate_route_candidates(
        corridors, stop_graph, stop_xy, node_ids=list(range(len(stops))),
    ...
    screened = sorted(
        ((surrogate_evaluator(demand, zone_xy, RouteSet([r]), config, zone_to_stop, stop_xy).score, r)
         for r in candidates),
        key=lambda x: x[0],
    )
    ...
    if full_assignment:
        ...
        def evaluator(route_set):
            ...
            ev = evaluate_route_set_aequilibrae(
                ...
                path_index=path_index, stop_to_zone=stop_to_zone,
                cache_dir=EVAL_CACHE,
            )
            return _with_coverage(ev, route_set, zone_xy, zones, stop_xy)

        def fast_evaluator(route_set):
            ev = surrogate_evaluator(demand, zone_xy, route_set, config, zone_to_stop, stop_xy)
            return _with_coverage(ev, route_set, zone_xy, zones, stop_xy)
    else:
        def evaluator(route_set):
            ev = surrogate_evaluator(demand, zone_xy, route_set, config, zone_to_stop, stop_xy)
    ...
    result = TNDPOptimizer(
        shortlist, evaluator, config, fast_evaluator=fast_evaluator, progress=progress
    ).solve(graph=stop_graph)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    route_path = save_route_set(result.routes, OUTPUT_DIR / "generated_routes.json")
    geojson_path = routes_to_geojson(
        result.routes, stops, OUTPUT_DIR / "generated_routes.geojson",
    ...
    report = {
        ...
        "base_score": float(meta.get("objective_base_score", ev.score)),
        "feasible": bool(meta.get("feasible", True)),
        "constraint_violations": meta.get("constraint_violations", []),
        "objective_components": meta.get("objective_components", {}),
        ...
        "operator_annual_mileage_km": float(ev.operator_cost),
        "capacity_excess": float(ev.capacity_excess),
        "annual_in_service_hours": float(meta.get("annual_in_service_hours", 0.0)),
        "fleet": int(meta.get("fleet", 0)),
        "annual_contract_cost_mln": float(meta.get("annual_contract_cost_mln", 0.0)),
        "annual_amortization_mln": float(meta.get("annual_amortization_mln", 0.0)),
        "coverage_400m": float(coverage.get("coverage_400m", 0.0)),
        "coverage_500m": float(coverage.get("coverage_500m", 0.0)),
        "coverage_800m": float(coverage.get("coverage_800m", 0.0)),
        "coverage_population": float(coverage.get("coverage_population", 0.0)),
        "route_characteristics": meta.get("route_characteristics", []),
