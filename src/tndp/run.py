...
from .validation import validate_interval_profile, validate_evaluation_metadata, validate_pipeline
...
def _load_inputs():
    ...
    stop_tree = cKDTree(stop_xy); _, zone_to_stops = stop_tree.query(zone_xy, k=min(4, len(stops))); zone_to_stops = np.atleast_2d(zone_to_stops)
    if zone_to_stops.shape[0] != len(zones): zone_to_stops = zone_to_stops.T
    zone_to_stop = zone_to_stops[:, 0].astype(int)
    zone_tree = cKDTree(zone_xy); _, stop_to_zone_idx = zone_tree.query(stop_xy, k=1)
    ...
    stop_demand = np.zeros(len(stops), dtype=float); zone_mass = zones["production"].to_numpy(float) + zones["attraction"].to_numpy(float); np.add.at(stop_demand, zone_to_stop, zone_mass)
    stop_lonlat = stops.to_crs("EPSG:4326"); stop_lonlat_xy = np.column_stack([stop_lonlat.geometry.x.to_numpy(float), stop_lonlat.geometry.y.to_numpy(float)])
...
def _empty_evaluation(demand, config):
    total = float(np.asarray(demand, dtype=float).sum()); return Evaluation(score=total * config.uncovered_demand_weight, uncovered_demand=total, direct_demand_share=0.0, metadata={"evaluator":"empty-network baseline"})

def _with_coverage(evaluation, route_set, zone_xy, zones, stop_xy):
    coverage = population_coverage(route_set, np.asarray(zone_xy)*1000.0, zones["population"].to_numpy(float), np.asarray(stop_xy)*1000.0, radii_m=(400.,500.,800.))
...
def run_tndp(config=None, *, full_assignment=True, progress=None):
    ...
    profile_check = validate_interval_profile(DEFAULT_INTERVAL_PROFILE).as_dict()
    notify(f"Профиль интервалов: {'OK' if profile_check['passed'] else 'ОШИБКА'}")
    ...
    zone_corridors=extract_demand_corridors(demand,zone_xy,top_pairs=config.corridor_top_pairs,max_distance_km=config.corridor_distance_km); corridors=_map_corridors_to_stops(zone_corridors,zone_to_stop); notify(f"Выделено {len(corridors)} OD-коридоров")
    ...
    if full_assignment:
        ...
        def evaluator(route_set):
            ...
            ev=evaluate_route_set_aequilibrae_periods(route_set,demand,stop_lonlat_xy,project_path,config,road_graph=road_graph,stop_mapping=stop_mapping,path_index=path_index,stop_to_zone=stop_to_zone,cache_dir=EVAL_CACHE,progress=notify)
    ...
    OUTPUT_DIR.mkdir(parents=True,exist_ok=True); route_path=save_route_set(result.routes,OUTPUT_DIR/"generated_routes.json"); geojson_path=routes_to_geojson(result.routes,stops,OUTPUT_DIR/"generated_routes.geojson",road_graph=road_graph,stop_to_road_node=stop_mapping); (OUTPUT_DIR/"history.json").write_text(json.dumps(result.history,ensure_ascii=False,indent=2),encoding="utf-8")
    ev=result.evaluation; meta=ev.metadata if isinstance(ev.metadata,dict) else {}; coverage=population_coverage(result.routes,zone_xy*1000.0,zones["population"].to_numpy(float),stop_xy*1000.0,radii_m=(400.,500.,800.))
    validation = validate_pipeline(DEFAULT_INTERVAL_PROFILE, route_plans=meta.get("unified_operating_plan", {}).get("routes", []), network_plan=meta.get("unified_operating_plan", {}), evaluation_metadata=meta).get("issues", [])
    report={"backend":"Tranmodel TNDP solver","demand_units":"transport zones","transit_units":"transit stops","network_units":"real road graph","n_zones":int(len(zones)),"n_stops":int(len(stops)),"n_terminals":int(len(terminal_nodes)),"n_corridors":int(len(zone_corridors)),"n_candidates":int(len(candidates)),"n_screened_candidates":int(len(shortlist)),"n_routes":int(result.routes.route_count()),"score":float(ev.score),"base_score":float(meta.get("objective_base_score",ev.score)),"feasible":bool(meta.get("feasible",True)),"constraint_violations":meta.get("constraint_violations",[]),"validation_issues":validation,"objective_components":meta.get("objective_components",{}),"user_cost":float(ev.user_cost),"direct_demand_share":float(ev.direct_demand_share),"uncovered_demand":float(ev.uncovered_demand),"transfers":float(ev.transfers),"operator_annual_mileage_km":float(ev.operator_cost),"capacity_excess":float(ev.capacity_excess),"annual_in_service_hours":float(meta.get("annual_in_service_hours",0.0)),"fleet":int(meta.get("fleet",meta.get("peak_fleet_reconciled",0)) or 0),"peak_fleet_reconciled":int(meta.get("peak_fleet_reconciled",0) or 0),"reconciled_annual_mileage_km":float(meta.get("reconciled_annual_mileage_km",0.0) or 0.0),"reconciled_annual_hours":float(meta.get("reconciled_annual_hours",0.0) or 0.0),"annual_total_cost_mln":float(meta.get("annual_total_cost_mln",0.0) or 0.0),"annual_contract_cost_mln":float(meta.get("annual_contract_cost_mln",0.0) or 0.0),"annual_amortization_mln":float(meta.get("annual_amortization_mln",0.0) or 0.0),"annual_fuel_energy_mln":float(meta.get("annual_fuel_energy_mln",0.0) or 0.0),"annual_repair_mln":float(meta.get("annual_repair_mln",0.0) or 0.0),"annual_crew_cost_mln":float(meta.get("annual_crew_cost_mln",0.0) or 0.0),"annual_infrastructure_mln":float(meta.get("annual_infrastructure_mln",0.0) or 0.0),"annual_dispatch_mln":float(meta.get("annual_dispatch_mln",0.0) or 0.0),"coverage_400m":float(coverage.get("coverage_400m",0.0)),"coverage_500m":float(coverage.get("coverage_500m",0.0)),"coverage_800m":float(coverage.get("coverage_800m",0.0)),"coverage_population":float(coverage.get("coverage_population",0.0)),"period_results":meta.get("period_results",[]),"unified_operating_plan":meta.get("unified_operating_plan",{}),"route_peak_reconciliation":meta.get("route_peak_reconciliation",[]),"route_characteristics":meta.get("route_characteristics",[]),"pareto_archive":result.pareto_archive or [],"route_set":str(route_path),"route_geojson":str(geojson_path),"evaluator":meta.get("evaluator","unknown"),"full_assignment":bool(full_assignment)}
    (OUTPUT_DIR/"tndp_report.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8"); notify(f"Синтез завершён: {result.routes.route_count()} маршрутов"); return report
