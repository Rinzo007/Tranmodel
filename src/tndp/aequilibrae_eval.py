...
def evaluate_route_set_aequilibrae(route_set: RouteSet, demand: np.ndarray, stop_xy_lonlat: np.ndarray, project_path: str | Path, config: NetworkDesignConfig, *, road_graph: nx.Graph, stop_mapping, path_index=None, stop_to_zone: dict[int, int] | None = None, cache_dir: str | Path | None = None, assignment_iteration: int = 0) -> Evaluation:
    ...
    try:
        ...
        weighted_user_cost = float(np.nansum(demand[finite] * generalized[finite]) / max(served, 1.0)); avg_transfers = float(np.nansum(demand * transfers_arr) / max(total, 1.0)); direct_share = float(demand[(finite) & (transfers_arr == 0)].sum() / max(total, 1.0))
        ...
        adapted = assigned_route_set; annual_mileage = annual_hours = annual_contract = annual_amortization = 0.0; fleet = 0; route_characteristics = []
        for i, route in enumerate(adapted.routes):
