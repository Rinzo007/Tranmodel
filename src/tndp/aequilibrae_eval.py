    ...
    *,
    ...
    try:
        ...
        builder = transit.new_gtfs_builder(
            agency="TRANMODEL", file_path=str(gtfs), day="2026-01-15", description="TNDP candidate route set"
        )
        # Raw GTFS shapes already follow the real road graph, so avoid a second
        # map-matching pass against the car-only GMNS network.
        ...
        evaluation = Evaluation(score=float(score), user_cost=weighted_user_cost, operator_cost=float(operator_km),
                                uncovered_demand=uncovered, transfers=avg_transfers,
