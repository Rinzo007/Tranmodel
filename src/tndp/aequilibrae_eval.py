...
def _adapt_fleet(route_set: RouteSet, demand: np.ndarray, stop_to_zone: dict[int, int], lengths: list[float], config: NetworkDesignConfig):
    """Fallback fleet adaptation used before exact AequilibraE assignment."""
    current = route_set
    details: list[dict] = []
    for _ in range(2):
    ...
    try:
        ...
        for i, route in enumerate(adapted.routes):
            ...
            if exact is not None:
                flow = float(exact["max_section_flow_pph"])
                max_idx = int(exact["max_section_index"])
                load_source = "AequilibraE TransitAssignmentResults"
            else:
                fallback = fleet_details[i] if i < len(fleet_details) else {}
                flow = float(fallback.get("max_section_flow_pph", _route_flow(route)))
                max_idx = int(fallback.get("max_section_index", -1))
                load_source = "reconstructed_fallback"
        ...
        evaluation = Evaluation(
            ...
            operator_cost=float(annual_mileage),
