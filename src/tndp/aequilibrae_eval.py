...
from .cost_aggregation import aggregate_network_costs, aggregate_route_costs
...
def _route_set_key(route_set: RouteSet) -> str:
    payload = {"version": EVALUATOR_VERSION, "routes": [{"nodes": list(r.nodes), "frequency_vph": _route_frequency(r), "max_section_flow_pph": _route_flow(r), "vehicle_type": getattr(r, "vehicle_type", "bus")} for r in route_set.routes]}
...
def _adapt_fleet(route_set: RouteSet, demand: np.ndarray, stop_to_zone: dict[int, int], lengths: list[float], config: NetworkDesignConfig):
    ...
    for _ in range(2):
        loads = reconstruct_route_loads(current, demand, stop_to_zone=stop_to_zone, route_lengths_km=lengths, frequencies_vph=[r.frequency_vph for r in current.routes])
        ...
        for i, route in enumerate(current.routes):
            ...
            code, op = select_vehicle_for_route(max_section_flow_pph=flow, route_length_km=max(.001, lengths[i] * 2), allowed_vehicle_types=config.allowed_vehicle_types, speed_kmh=config.speed_kmh, interval_reserve_sec=config.interval_reserve_sec, terminal_delay_reserve=config.terminal_delay_reserve, charging_min_per_terminal=config.charging_min_per_terminal, annual_days=config.annual_days, park_trip_coefficient=config.park_trip_coefficient, frequency_profile=None)
            nr = route.with_flow(flow).with_vehicle_type(code).with_frequency(float(op["frequency_vph"]))
            changed |= nr != route; updated.append(nr); details.append({"max_section_flow_pph": flow, "max_section_index": loads[i].max_section_index, "assigned_demand": loads[i].assigned_demand, **op})
...
def _routes_changed(before: RouteSet, after: RouteSet, frequency_tolerance: float = 0.01) -> bool:
    ...
    return any(tuple(a.nodes) != tuple(b.nodes) or getattr(a, "vehicle_type", "bus") != getattr(b, "vehicle_type", "bus") or abs(_route_frequency(a) - _route_frequency(b)) > frequency_tolerance for a, b in zip(before.routes, after.routes))

def evaluate_route_set_aequilibrae(route_set: RouteSet, demand: np.ndarray, stop_xy_lonlat: np.ndarray, project_path: str | Path, config: NetworkDesignConfig, *, road_graph: nx.Graph, stop_mapping, path_index=None, stop_to_zone: dict[int, int] | None = None, cache_dir: str | Path | None = None, assignment_iteration: int = 0) -> Evaluation:
    total = float(np.asarray(demand, dtype=float).sum())
    if not route_set.routes: return Evaluation(score=total * config.uncovered_demand_weight, uncovered_demand=total, metadata={"empty_network": True})
    ...
    key = _route_set_key(adapted); root = Path(cache_dir) if cache_dir else Path(tempfile.mkdtemp(prefix="tranmodel_tndp_eval_")); root.mkdir(parents=True, exist_ok=True)
    ...
    except ImportError as exc: raise AequilibraEEvaluationError("AequilibraE is required for full TNDP evaluation") from exc
    ...
    try:
        ...
        assignment.set_time_field("trav_time"); assignment.set_frequency_field("freq"); assignment.set_skimming_fields(["trav_time", "on_board_trav_time", "walking_trav_time", "waiting_time", "transfer_time", "boardings", "transfers"]); assignment.set_algorithm("os"); transit_class.set_demand_matrix_core("pt"); assignment.execute()
        ...
        for i, route in enumerate(adapted.routes):
            ...
            flow = float(exact["max_section_flow_pph"]) if exact is not None else float(fallback.get("max_section_flow_pph", _route_flow(route))); max_idx = int(exact["max_section_index"]) if exact is not None else int(fallback.get("max_section_index", -1)); load_source = "AequilibraE TransitAssignmentResults" if exact is not None else "reconstructed_fallback"
            code, op = select_vehicle_for_route(max_section_flow_pph=flow, route_length_km=max(.001, lengths[i] * 2), allowed_vehicle_types=config.allowed_vehicle_types, speed_kmh=config.speed_kmh, interval_reserve_sec=config.interval_reserve_sec, terminal_delay_reserve=config.terminal_delay_reserve, charging_min_per_terminal=config.charging_min_per_terminal, annual_days=config.annual_days, park_trip_coefficient=config.park_trip_coefficient, frequency_profile=None)
            final_routes.append(route.with_flow(flow).with_vehicle_type(code).with_frequency(float(op["frequency_vph"]))); final_details.append({"max_section_flow_pph": flow, "max_section_index": max_idx, "load_source": load_source, **op})
        ...
        if assignment_iteration < MAX_ASSIGNMENT_FLEET_ITERATIONS - 1 and _routes_changed(adapted, assigned_route_set):
            return evaluate_route_set_aequilibrae(assigned_route_set, demand, stop_xy_lonlat, project_path, config, road_graph=road_graph, stop_mapping=stop_mapping, path_index=path_index, stop_to_zone=stop_to_zone, cache_dir=cache_dir, assignment_iteration=assignment_iteration + 1)
        annual_route_costs = []; fleet = 0
        route_characteristics = []
        annual_mileage = annual_hours = annual_contract = annual_amortization = 0.0
        for i, route in enumerate(assigned_route_set.routes):
            one_way = lengths[i]; d = final_details[i]; vehicle = get_vehicle_type(route.vehicle_type)
            operating = calculate_route_characteristics(2 * one_way, d["max_section_flow_pph"], capacity_at_4_ppm2=route.capacity, speed_kmh=config.speed_kmh, interval_reserve_sec=config.interval_reserve_sec, terminal_delay_reserve=config.terminal_delay_reserve, charging_min_per_terminal=config.charging_min_per_terminal, charging_at_terminal=vehicle.charging_at_terminal, technical_readiness=route.technical_readiness, frequency_profile=None)
            annual_mileage += operating.annual_mileage_km; annual_hours += operating.annual_in_service_hours; fleet += operating.fleet; annual_contract += float(d.get("annual_fleet_contract_cost_mln", 0.0)); annual_amortization += float(d.get("annual_fleet_amortization_mln", 0.0))
            route_cost = aggregate_route_costs(vehicle_type=route.vehicle_type, annual_km=operating.annual_mileage_km, fleet=operating.fleet, annual_hours=operating.annual_in_service_hours, annual_contract_mln=float(d.get("annual_fleet_contract_cost_mln", 0.0)), annual_amortization_mln=float(d.get("annual_fleet_amortization_mln", 0.0)))
            route_cost["fleet"] = operating.fleet; route_cost["annual_mileage_km"] = operating.annual_mileage_km; route_cost["annual_hours"] = operating.annual_in_service_hours; annual_route_costs.append(route_cost)
            route_characteristics.append({"route_id": route.route_id, "one_way_length_km": one_way, "round_trip_length_km": operating.route_length_km, "max_section_flow_pph": d["max_section_flow_pph"], "frequency_vph": operating.frequency_vph, "interval_min": operating.interval_min, "turnaround_min": operating.turnaround_min, "release": operating.release, "technical_readiness": operating.technical_readiness, "fleet": operating.fleet, "daily_trips": operating.daily_trips, "annual_mileage_km": operating.annual_mileage_km, "annual_in_service_hours": operating.annual_in_service_hours, "vehicle_type": route.vehicle_type, "vehicle_name": d.get("vehicle_name", ""), "capacity": route.capacity, "annual_contract_cost_mln": d.get("annual_fleet_contract_cost_mln", 0.0), "annual_amortization_mln": d.get("annual_fleet_amortization_mln", 0.0), "max_section_index": d["max_section_index"], "load_source": d["load_source"], "annual_costs": route_cost})
        costs = aggregate_network_costs(annual_route_costs)
        annual_contract = costs["contract_mln"]; annual_amortization = costs["amortization_mln"]
        score = weighted_user_cost + annual_contract + annual_amortization + costs["total_annual_mln"] * config.objective_operating_weight + uncovered * config.uncovered_demand_weight + avg_transfers * config.transfer_penalty_min * config.transfer_weight
        evaluation = Evaluation(score=float(score), user_cost=weighted_user_cost, operator_cost=float(costs["total_annual_mln"]), uncovered_demand=uncovered, transfers=avg_transfers, direct_demand_share=direct_share, metadata={"evaluator": "AequilibraE", "served_demand": served, "annual_mileage_km": annual_mileage, "annual_in_service_hours": annual_hours, "fleet": fleet, "annual_contract_cost_mln": annual_contract, "annual_amortization_mln": annual_amortization, "annual_fuel_energy_mln": costs["fuel_energy_mln"], "annual_repair_mln": costs["repair_mln"], "annual_crew_cost_mln": costs["crew_mln"], "annual_infrastructure_mln": costs["infrastructure_mln"], "annual_dispatch_mln": costs["dispatch_mln"], "annual_total_operating_cost_mln": costs["total_annual_mln"], "route_characteristics": route_characteristics, "assignment_segment_loads": exact_loads, "segment_load_source": "AequilibraE TransitAssignmentResults", "assignment_iteration": assignment_iteration + 1, "assignment_fleet_converged": True, "assignment_fleet_max_iterations": MAX_ASSIGNMENT_FLEET_ITERATIONS})
