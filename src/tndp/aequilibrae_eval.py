...
def build_gtfs_from_route_set(route_set: RouteSet, stop_xy_lonlat: np.ndarray,
    ...
    """Build deterministic GTFS for a candidate route set."""
    ...
    try:
        ...
        agency = [{"agency_id": "TRANMODEL", "agency_name": "Tranmodel TNDP",
                   ...
                   "agency_timezone": "Europe/Moscow", "agency_lang": "ru"}]
        ...
        for route_num, route in enumerate(route_set.routes, start=1):
            route_rows.append({"route_id": f"r{route_num}", "agency_id": "TRANMODEL",
                               "route_short_name": str(route.route_id or route_num),
                               "route_long_name": f"TNDP route {route_num}", "route_type": 3})
            frequency = max(float(route.frequency_vph), 60.0 / default_headway_min)
            ...
            for direction_id, seq in enumerate((tuple(route.nodes), tuple(reversed(route.nodes)))):
                ...
                for trip_no, departure in enumerate(departures, start=1):
                    ...
                    trip_rows.append({"route_id": f"r{route_num}", "service_id": "daily",
                                      "trip_id": trip_id, "trip_headsign": f"direction {direction_id + 1}",
                    ...
                    for stop_seq, node in enumerate(seq, start=1):
                        if stop_seq > 1:
                            ...
                            elapsed += _distance_m((float(a[0]), float(a[1])),
                                                   (float(b[0]), float(b[1]))) / (speed_kmh * 1000.0 / 3600.0)
                            elapsed += dwell_sec
...
def evaluate_route_set_aequilibrae(route_set: RouteSet, demand_matrix: np.ndarray,
    ...
    """Evaluate a route set with AequilibraE TransitAssignment/Optimal Strategies."""
    ...
    try:
        ...
        if "transfers" in skim:
            generalized += (np.nan_to_num(np.asarray(skim["transfers"]), nan=np.inf)
                             * config.transfer_penalty_min * config.transfer_weight)
        ...
        operator_km = sum(
            _distance_m((float(stop_xy_lonlat[a, 0]), float(stop_xy_lonlat[a, 1])),
                        (float(stop_xy_lonlat[b, 0]), float(stop_xy_lonlat[b, 1]))) / 1000.0
            for route in route_set.routes for a, b in zip(route.nodes[:-1], route.nodes[1:])
        )
