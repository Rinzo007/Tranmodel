...
EVALUATOR_VERSION = "aeq-transit-v4-real-road"
...
def _route_length_km(route: Any, road_graph: nx.Graph, stop_mapping) -> float:
    ...
    for a, b in zip(route.nodes[:-1], route.nodes[1:]):
        try:
            ...
            raise AequilibraEEvaluationError(f"No road path between stops {a} and {b}") from exc
    return total
...
def _route_set_key(route_set: RouteSet) -> str:
    payload = {
        ...
        "routes": [{"nodes": list(route.nodes), "frequency_vph": _route_frequency(route)} for route in route_set.routes],
    }
...
def evaluate_route_set_aequilibrae(route_set: RouteSet, demand: np.ndarray, stop_xy_lonlat: np.ndarray,
    ...
    try:
        ...
        builder = transit.new_gtfs_builder(agency="TRANMODEL", file_path=str(gtfs), day="2026-01-15",
                                           description="TNDP candidate route set")
        builder.set_allow_map_match(False)
        builder.load_date("2026-01-15")
        builder.save_to_disk()

        graph_builder = transit.create_graph(projected_crs=f"EPSG:{PROJ_EPSG}",
            with_inner_stop_transfers=True, with_outer_stop_transfers=False, with_walking_edges=True,
            distance_upper_bound=800.0, blocking_centroid_flows=True, connector_method="nearest_neighbour",
        ...
        evaluation = Evaluation(score=float(score), user_cost=weighted_user_cost, operator_cost=float(operator_km), uncovered_demand=uncovered,
                                ...
                                metadata={"evaluator": "AequilibraE", "evaluator_version": EVALUATOR_VERSION, "served_demand": served})
