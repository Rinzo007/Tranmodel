...
def _route_length_km(route: Any, road_graph: nx.Graph, stop_mapping) -> float:
    """Calculate route length on the real road graph."""
    total = 0.0
    for a, b in zip(route.nodes[:-1], route.nodes[1:]):
        try:
            ...
            raise AequilibraEEvaluationError(
                f"No road path between stops {a} and {b}"
            ) from exc
    return total
    ...
    *,
    ...
    try:
        ...
        for field, weight in (
            ...
            if field in skim:
                generalized += np.nan_to_num(
                    np.asarray(skim[field]), nan=np.inf
                ) * weight
        ...
        operator_km = sum(
            _route_length_km(route, road_graph, stop_mapping) * _route_frequency(route)
            for route in route_set.routes
        )
        score = (
            weighted_user_cost
