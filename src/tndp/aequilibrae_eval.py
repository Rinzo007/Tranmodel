...
EVALUATOR_VERSION = "aeq-transit-v16-period-gtfs-uncovered-is-boardings"
...
def _profile_key(profile: Iterable[IntervalPeriod]) -> tuple:
    return tuple(
        (p.number, p.name, p.start, p.end, float(p.frequency_factor), float(p.hours))
        for p in profile
    )


def _route_set_key(route_set: RouteSet, service_profile=DEFAULT_INTERVAL_PROFILE) -> str:
    payload = {
        "version": EVALUATOR_VERSION,
        "service_profile": _profile_key(service_profile),
        "routes": [
            {
                "nodes": list(r.nodes),
                "frequency_vph": _route_frequency(r),
                "max_section_flow_pph": _route_flow(r),
                "vehicle_type": getattr(r, "vehicle_type", "bus"),
            }
...
def _adapt_fleet(route_set: RouteSet, demand: np.ndarray, stop_to_zone: dict[int, int], lengths: list[float], config: NetworkDesignConfig):
    ...
    details: list[dict] = []
    for _ in range(2):
        loads = reconstruct_route_loads(
            current,
            demand,
            stop_to_zone=stop_to_zone,
            route_lengths_km=lengths,
        ...
        for i, route in enumerate(current.routes):
            flow = max(
                loads[i].max_section_flow_pph if i < len(loads) else 0.0,
                _route_flow(route),
            )
            code, op = select_vehicle_for_route(
                max_section_flow_pph=flow,
                route_length_km=max(.001, lengths[i] * 2),
            ...
            details.append(
                {
                    "max_section_flow_pph": flow,
                    "max_section_index": loads[i].max_section_index,
                    "assigned_demand": loads[i].assigned_demand,
                    **op,
                }
            )
...
def _routes_changed(before: RouteSet, after: RouteSet, frequency_tolerance: float = 0.01) -> bool:
    if before.route_count() != after.route_count():
        return True
    return any(
        tuple(a.nodes) != tuple(b.nodes)
        or getattr(a, "vehicle_type", "bus") != getattr(b, "vehicle_type", "bus")
        or abs(_route_frequency(a) - _route_frequency(b)) > frequency_tolerance
...
def evaluate_route_set_aequilibrae(
    ...
    *,
    ...
    service_profile: Iterable[IntervalPeriod] = DEFAULT_INTERVAL_PROFILE,
) -> Evaluation:
    """Evaluate one route set for one explicit service-frequency profile.

    The caller supplies the already period-specific route frequencies. The GTFS
    builder receives the same period window with a neutral frequency factor, so
    the frequency factor is applied exactly once by _period_route_set().
    """
    service_profile = tuple(service_profile)
    if not service_profile:
        raise ValueError("service_profile cannot be empty")
    ...
    adapted, fleet_details = (
        _adapt_fleet(route_set, demand, stop_to_zone, lengths, config)
        if stop_to_zone
        else (route_set, [])
    )
    key = _route_set_key(adapted, service_profile)
    ...
    try:
        ...
        raise AequilibraEEvaluationError(
            "AequilibraE is required for full TNDP evaluation"
        ) from exc
    ...
    try:
        ...
        if public_db.exists():
            public_db.unlink()
        gtfs = build_gtfs_from_route_set(
            adapted,
            stop_xy_lonlat,
            temp_root / "routes.zip",
            road_graph=road_graph,
            stop_mapping=stop_mapping,
            path_index=path_index,
            interval_profile=service_profile,
        )
        ...
        def field_array(name: str, default: float = 0.0):
            try:
                return np.asarray(skim[name], dtype=float)
            except Exception:
                return np.full_like(demand, default, dtype=float)
        ...
        generalized = (
            config.in_vehicle_weight * ride
            + config.wait_weight * wait
            + config.walk_weight * walk
            + config.transfer_weight * transfers_arr * config.transfer_penalty_min
        )
        served = float(demand[served_mask].sum())
        uncovered = float(total - served)
        weighted_user_cost = float(
            np.nansum(demand[served_mask] * generalized[served_mask]) / max(served, 1.0)
        )
        avg_transfers = float(
            np.nansum(demand[served_mask] * transfers_arr[served_mask]) / max(served, 1.0)
        )
        direct_share = float(
            demand[served_mask & (transfers_arr == 0)].sum() / max(total, 1.0)
        )
        ...
        for i, route in enumerate(adapted.routes):
            exact = exact_max.get(i)
            fallback = fleet_details[i] if i < len(fleet_details) else {}
            flow = (
                float(exact["max_section_flow_pph"])
                if exact is not None
                else float(fallback.get("max_section_flow_pph", _route_flow(route)))
            )
            max_idx = (
                int(exact["max_section_index"])
                if exact is not None
                else int(fallback.get("max_section_index", -1))
            )
            load_source = (
                "AequilibraE TransitAssignmentResults"
                if exact is not None
            ...
            final_routes.append(
                route.with_flow(flow).with_vehicle_type(code).with_frequency(float(op["frequency_vph"]))
            )
        ...
        for i, route in enumerate(assigned_route_set.routes):
            one_way = lengths[i]
            d = final_details[i]
            vehicle = get_vehicle_type(route.vehicle_type)
            operating = calculate_route_characteristics(
                2 * one_way,
                d["max_section_flow_pph"],
                capacity_at_4_ppm2=route.capacity,
                speed_kmh=config.speed_kmh,
                interval_reserve_sec=config.interval_reserve_sec,
                terminal_delay_reserve=config.terminal_delay_reserve,
                charging_min_per_terminal=config.charging_min_per_terminal,
            ...
            route_cost = aggregate_route_costs(
                ...
                annual_km=operating.annual_mileage_km,
                fleet=operating.fleet,
                annual_hours=operating.annual_in_service_hours,
                annual_contract_mln=float(d.get("annual_fleet_contract_cost_mln", 0.0)),
                annual_amortization_mln=float(d.get("annual_fleet_amortization_mln", 0.0)),
            )
            route_cost["fleet"] = operating.fleet
            route_cost["annual_mileage_km"] = operating.annual_mileage_km
            route_cost["annual_hours"] = operating.annual_in_service_hours
            annual_route_costs.append(route_cost)
            route_characteristics.append(
                {
                    "route_id": route.route_id,
                    "one_way_length_km": one_way,
                    "round_trip_length_km": operating.route_length_km,
                    "max_section_flow_pph": d["max_section_flow_pph"],
                    ...
                    "max_section_index": d["max_section_index"],
                    "load_source": d["load_source"],
                    "annual_costs": route_cost,
                }
            )
        costs = aggregate_network_costs(annual_route_costs)
        annual_contract = costs["contract_mln"]
        annual_amortization = costs["amortization_mln"]
        score = (
            weighted_user_cost
            + annual_contract
            + annual_amortization
            + costs["total_annual_mln"] * config.objective_operating_weight
        ...
        evaluation = Evaluation(
            ...
            operator_cost=float(costs["total_annual_mln"]),
            ...
            metadata={
                ...
                "fleet": fleet,
                "annual_contract_cost_mln": annual_contract,
                "annual_amortization_mln": annual_amortization,
                "annual_fuel_energy_mln": costs["fuel_energy_mln"],
                "annual_repair_mln": costs["repair_mln"],
                "annual_crew_cost_mln": costs["crew_mln"],
                "annual_infrastructure_mln": costs["infrastructure_mln"],
                "annual_dispatch_mln": costs["dispatch_mln"],
                "annual_total_operating_cost_mln": costs["total_annual_mln"],
                ...
                "assignment_iteration": assignment_iteration + 1,
                "assignment_fleet_converged": True,
