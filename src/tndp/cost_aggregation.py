...
def aggregate_route_costs(*, vehicle_type: str, annual_km: float, fleet: int,
    ...
    total = (
        base["total_before_vehicle"]
        + float(annual_contract_mln)
        + float(annual_amortization_mln)
    )
    return {
        **base,
        "contract_mln": float(annual_contract_mln),
        "amortization_mln": float(annual_amortization_mln),
...
def aggregate_network_costs(route_costs: list[dict]) -> dict[str, float]:
    keys = ("fuel_energy_mln", "repair_mln", "crew_mln", "infrastructure_mln",
    ...
    result["annual_mileage_km"] = sum(float(r.get("annual_mileage_km", 0.0)) for r in route_costs)
