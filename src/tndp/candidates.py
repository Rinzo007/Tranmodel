...
def _feasible(path: list[int], length_km: float, config: NetworkDesignConfig) -> bool:
    return config.min_stops <= len(path) <= config.max_stops and config.min_route_length_km <= length_km <= config.max_route_length_km


def _insert_high_demand_nodes(graph: nx.Graph, base_path: list[int], demand_vector: np.ndarray,
                              node_to_idx: dict[int, int], config: NetworkDesignConfig) -> list[list[int]]:
    variants = [base_path]
    ...
    for node in candidates:
        ...
        if best_pos is not None:
            variants.append(base_path[:best_pos] + [int(node)] + base_path[best_pos:])
