...
def combine_objective(c: ObjectiveComponents, config: NetworkDesignConfig) -> float:
    return (
        ...
        + (c.fuel_energy_cost + c.repair_cost + c.crew_cost + c.infrastructure_cost + c.dispatch_cost)
        * config.objective_operating_weight
    )
...
def evaluation_from_components(c: ObjectiveComponents, config: NetworkDesignConfig, *, metadata=None) -> Evaluation:
    return Evaluation(score=combine_objective(c, config), user_cost=c.user_cost, operator_cost=c.operator_cost,
                      uncovered_demand=c.uncovered_demand, transfers=c.transfers, capacity_excess=c.capacity_excess,
...
def apply_objective(route_set: RouteSet, evaluation: Evaluation, config: NetworkDesignConfig) -> Evaluation:
    ...
    unified = meta.get("unified_operating_plan") or {}
    unified_costs = unified.get("costs") or {}
    rc = meta.get("route_characteristics") or []

    def sum_route(key: str) -> float:
        return sum(float(x.get(key, 0.0) or 0.0) for x in rc if isinstance(x, dict))

    # Prefer the reconciled six-period network plan. Fall back to legacy
    # metadata so older surrogate/full evaluations remain compatible.
    contract = float(meta.get("annual_contract_cost_mln", unified_costs.get("contract_mln", sum_route("annual_contract_cost_mln"))) or 0.0)
    amort = float(meta.get("annual_amortization_mln", unified_costs.get("amortization_mln", sum_route("annual_amortization_mln"))) or 0.0)
    fuel = float(meta.get("annual_fuel_energy_mln", unified_costs.get("fuel_energy_mln", sum_route("annual_fuel_energy_mln"))) or 0.0)
    repair = float(meta.get("annual_repair_mln", unified_costs.get("repair_mln", sum_route("annual_repair_mln"))) or 0.0)
    crew = float(meta.get("annual_crew_cost_mln", unified_costs.get("crew_mln", sum_route("annual_crew_cost_mln"))) or 0.0)
    infra = float(meta.get("annual_infrastructure_mln", unified_costs.get("infrastructure_mln", sum_route("annual_infrastructure_mln"))) or 0.0)
    dispatch = float(meta.get("annual_dispatch_mln", unified_costs.get("dispatch_mln", sum_route("annual_dispatch_mln"))) or 0.0)
    annual_total = float(meta.get("annual_total_cost_mln", unified_costs.get("total_annual_mln", fuel + repair + crew + infra + dispatch + contract + amort)) or 0.0)
    ...
    c = ObjectiveComponents(
        max(0, float(evaluation.user_cost)), max(0, float(evaluation.operator_cost)),
        max(0, float(evaluation.uncovered_demand)), max(0, float(evaluation.transfers)),
        max(0, float(evaluation.capacity_excess)), duplication, max(0, walk), max(0, wait),
        max(0, contract), max(0, amort), max(0, fuel), max(0, repair), max(0, crew), max(0, infra), max(0, dispatch)
    )
    violations = []
    ...
    fleet = int(float(meta.get("fleet", unified.get("fleet", 0)) or 0))
    ...
    meta.update({
        "objective_components": asdict(c), "objective_base_score": base, "objective_penalty": penalty,
        "feasible": not violations, "constraint_violations": violations,
        "annual_fuel_energy_mln": fuel, "annual_repair_mln": repair, "annual_crew_cost_mln": crew,
        "annual_infrastructure_mln": infra, "annual_dispatch_mln": dispatch,
        "annual_total_cost_mln": annual_total,
        "economic_source": "unified_operating_plan" if unified_costs else "legacy_route_metadata",
    })
